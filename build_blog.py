...
.post-day {
    ...
    background: linear-gradient(135deg, var(--primary), var(--secondary));
    color: white; width: 50px; height: 50px; line-height: 50px;
    text-align: center; border-radius: 12px; font-size: 1.3em; font-weight: 700; margin-bottom: 15px;
}
...
.skill-tag {
    ...
    font-size: 0.85em; font-weight: 500; border: 1px solid rgba(108,92,231,0.2);
}
    ...
    for f in sorted(POSTS_DIR.glob("*.md")):
        ...
        for line in lines:
            if '心情：' in line or 'mood:' in line.lower():
                if any(k in line for k in ['成长','growth']): mood = '成长'
                elif any(k in line for k in ['忙碌','busy']): mood = '忙碌'
                elif any(k in line for k in ['突破','breakthrough']): mood = '突破'
                elif any(k in line for k in ['学习','learn']): mood = '学习'
...
<title>归档 - {title}</title><link rel="stylesheet" href="css/style.css"></head>
<body>
...
<footer><p>📝 {title} · {author}</p></footer>
        ...
        title=BLOG_CONFIG["title"], author=BLOG_CONFIG["author"], count=len(posts), archive_html=archive_html)
...
def build_skills(posts):
    ...
    return render_html("""<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>技能树 - {title}</title><link rel="stylesheet" href="css/style.css">
...
def build_about():
    return render_html("""<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>关于 - {title}</title><link rel="stylesheet" href="css/style.css"></head>
<body>
...
def build_single_post(post):
    ...
    return render_html("""<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>{title} - {blog_title}</title><link rel="stylesheet" href="../css/style.css"></head>
<body>
...
<footer><p>📝 {blog_title} · {author} · {date}</p></footer>
        ...
        title=post['title'], blog_title=BLOG_CONFIG["title"],
        blog_subtitle=BLOG_CONFIG["subtitle"], author=BLOG_CONFIG["author"],
        date=post['date_str'], day=post['day'], weekday=post['weekday'],
        post_title=post['title'], mood_tags=mood_tags,
        skill_tags=skill_tags, content=post['html'])
...
def generate_blog():
    ...
    print(f"✅ {len(posts)} 篇单文章页已生成")

    print(f"\n🎉 博客生成完成！共 {len(posts)} 篇文章")
